...
function runEngineCheck(databaseUrl?: string) {
  ...
  const result = spawnSync(
    ...
    [
      ...
      "tsx",
      "-e",
      "import { getDatabaseEngine, getDatabaseClient } from './src/lib/server/database-client.ts'; console.log(getDatabaseEngine()); try { getDatabaseClient(); console.log('client-ok'); } catch { console.log('client-guarded'); }"
    ],
    {
      cwd: process.cwd(),
      env,
