...
import { createPostgresQueryAdapter } from "@/shared/lib/server/query-adapter-pg";
...
async function main() {
  ...
  try {
    ...
    for (const table of TABLES) {
      const sqliteColumns = (sqlite.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>).map((c) => c.name);
      if (!sqliteColumns.length) {
        continue;
      }
      const targetColumnsResult = await pool.query(
        "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = $1 ORDER BY ordinal_position",
        [table]
      );
      const targetColumns = new Set(targetColumnsResult.rows.map((row) => String(row.column_name)));
      const columns = sqliteColumns.filter((column) => targetColumns.has(column));
      if (!columns.length) {
        continue;
      }
