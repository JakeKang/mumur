...
function initSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS webhook_deliveries (
      ...
      FOREIGN KEY(webhook_id) REFERENCES team_webhooks(id) ON DELETE CASCADE,
      FOREIGN KEY(event_id) REFERENCES events(id) ON DELETE CASCADE
    );

    CREATE TABLE IF NOT EXISTS team_navigator_views (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      team_id INTEGER NOT NULL,
      name TEXT NOT NULL,
  ...
  const teamViewColumns = db.prepare("PRAGMA table_info(team_navigator_views)").all();
  const hasUpdatedBy = teamViewColumns.some((column) => column.name === "updated_by");
  if (!hasUpdatedBy) {
    db.prepare("ALTER TABLE team_navigator_views ADD COLUMN updated_by INTEGER").run();
  }
  db.prepare("UPDATE team_navigator_views SET updated_by = created_by WHERE updated_by IS NULL").run();
}
