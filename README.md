...
| 영역       | 기술                                                                                                  |
| ---------- | ----------------------------------------------------------------------------------------------------- |
| 프레임워크 | [Next.js 16](https://nextjs.org) (App Router) + `server.ts` 커스텀 런타임                             |
| 언어       | TypeScript 5                                                                                          |
| 스타일     | [Tailwind CSS 4](https://tailwindcss.com) + 커스텀 UI 프리미티브                                      |
| 상태/데이터 | [@tanstack/react-query](https://tanstack.com/query) + SQLite 기본 경로, PostgreSQL 전환/검증 스크립트 |
| 실시간 협업 | [Yjs](https://yjs.dev) + [ws](https://github.com/websockets/ws)                                       |
| Markdown   | [marked](https://marked.js.org) + [highlight.js](https://highlightjs.org)                             |
| 테스트     | [Playwright](https://playwright.dev) + [Vitest](https://vitest.dev)                                   |
...
```bash
pnpm dev            # tsx watch 기반 개발 서버 (port 3100)
...
pnpm run release:verify # 릴리즈 전 타입체크, E2E, 빌드, PG 드라이런/롤백 검증 일괄 실행
```
...
```
server.ts                     Next.js 앱 진입점 겸 커스텀 서버 런타임
src/
  app/
    (auth)/                  인증 관련 라우트 그룹
    api/                     API 라우트
  ...
  components/
    ...
    ui/                      공통 UI 프리미티브
  db/
    migrations/             SQLite->PostgreSQL 전환/롤백 스크립트
    queries/                DB 어댑터 패리티 검증 로직
    seeds/                  로컬 계정/개발 데이터 시드
  features/
    auth/                   인증 도메인
    ideas/                  아이디어·스레드·협업 기능
    notifications/          알림 도메인
    workspace/              워크스페이스 도메인
  lib/
    server/                  서버 전용 유틸 디렉터리
  modules/
    workbench/              워크벤치 조합 계층
  shared/
    components/             공용 컴포넌트
    hooks/                  공용 훅
    lib/                    공용 라이브러리
...
e2e/
  auth.spec.ts               인증 E2E
  collab-socket.spec.ts      실시간 협업 소켓 E2E
  idea-thread.spec.ts        에디터 · 협업 E2E
  security-api.spec.ts       인증/보안 API 회귀 E2E
...
- `src/db/schema.sql` 기준으로 PostgreSQL 스키마 준비
- `src/db/migrations/*` 이관 흐름으로 데이터 마이그레이션, 패리티 검증, 롤백 드릴 수행
- `src/db/queries/adapter-parity.ts`로 SQLite/PG 쿼리 패리티 확인
- 배포 환경변수: `DATABASE_URL` + SSL 옵션 적용

### PostgreSQL 마이그레이션/검증 스크립트

- `src/db/schema.sql`: PostgreSQL 대상 스키마 기준 파일
- `src/db/migrations/migrate-sqlite-to-postgres.ts`: SQLite 데이터 이관 스크립트
- `src/db/migrations/validate-parity.ts`: 테이블 카운트 패리티 검증
- `src/db/migrations/dry-run-pgmem.ts`: pg-mem 기반 로컬 드라이런(이관+패리티+어댑터 핵심 검증)
- `src/db/migrations/rollback-drill.ts`: 엔진 전환/롤백 가드 검증
...
- 배포 후 `pnpm run release:verify` 또는 필요한 게이트를 순서에 맞게 재검증
...
<div align="center">
  <sub>Next.js · TypeScript · Tailwind CSS · React Query · SQLite/PostgreSQL · Playwright · Vitest 기반으로 구축되었습니다.</sub>
</div>
